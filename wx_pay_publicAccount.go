...
type WxPublicAccountOrderReq struct {
...
type WxPublicAccountOrderResp struct {
...
type pay_wxpay_publicAccount struct {
	config map[string]interface{}
}

func (this *pay_wxpay_publicAccount) Init(conf map[string]interface{}) {
	this.config = conf

}

func (this *pay_wxpay_publicAccount) Create_Order(shop_name string, shop_body string, money int, order_no string) (map[string]interface{}, error) {
	var xmlReq WxPublicAccountOrderReq
	...
	m["openid"] = xmlReq.Openid
	xmlReq.Sign = this.WxpayCalcSign(m, this.config["apikey"].(string)) //这个是计算wxpay签名的函数上面已贴出

	bytes_req, err := xml.Marshal(xmlReq)

	if err != nil {
		Log.Write("[微信公众号支付]XMl编码错误,错误原因"+err.Error(), LogErr)

		return make(map[string]interface{}), errors.New("[微信公众号支付]XMl编码错误,错误原因" + err.Error())
	}
	...
	if err != nil {
		...
		Log.Write("[微信公众号支付]Http Request失败,错误原因"+err.Error(), LogErr)
		return make(map[string]interface{}), errors.New("[微信公众号支付]Http Request失败,错误原因" + err.Error())
	}
	...
	if _err != nil {
		Log.Write("[微信公众号支付]请求微信支付统一下单接口发送错误,错误原因"+_err.Error(), LogErr)
		return make(map[string]interface{}), errors.New("[微信公众号支付]请求微信支付统一下单接口发送错误,错误原因" + _err.Error())
	}
	bodyByte, _ := ioutil.ReadAll(resp.Body)
	body := bodyByte
	fmt.Println(string(body))
	var xmlResp WxPublicAccountOrderResp
	...
	if xmlResp.Return_code == "SUCCESS" {
		...
		returnMap["signType"] = "MD5"
		returnMap["package"] = `prepay_id=` + xmlResp.Prepay_id
		returnMap["paySign"] = this.WxpayCalcSign(returnMap, this.config["apikey"].(string))
		Log.Write("[微信公众号支付] 订单"+order_no+"生成完毕", LogInfo)
		return returnMap, nil

	}

	Log.Write("[微信公众号支付]统一下单接口返回错误,错误原因"+xmlResp.Return_msg, LogErr)
	return make(map[string]interface{}), errors.New("[微信公众号支付]统一下单接口返回错误,错误原因" + xmlResp.Return_msg)

}
func (this *pay_wxpay_publicAccount) Refund() {

}

type WxPublicAccountNotifyReq struct {
	Return_code    string `xml:"return_code"`
	Return_msg     string `xml:"return_msg"`
	Appid          string `xml:"appid"`
	Mch_id         string `xml:"mch_id"`
...
type WxPublicAccountNotifyResp struct {
	Return_code string `xml:"return_code"`
	Return_msg  string `xml:"return_msg"`
}

func (t *pay_wxpay_publicAccount) Callback(w http.ResponseWriter, r *http.Request) bool {

	body, read_err := ioutil.ReadAll(r.Body)
	if read_err != nil {
		//fmt.Println("读取http body失败，原因!", read_err)
		Log.Write("[微信公众号支付]读取http body失败,错误原因"+read_err.Error(), LogErr)
		http.Error(w.(http.ResponseWriter), http.StatusText(http.StatusBadRequest), http.StatusBadRequest)
		return false
	}
	return_type := false
	var mr WxPublicAccountNotifyReq
	err := xml.Unmarshal(body, &mr)
	if err != nil {

		Log.Write("[微信公众号支付]解析XML失败,错误原因"+err.Error(), LogErr)
		http.Error(w.(http.ResponseWriter), http.StatusText(http.StatusBadRequest), http.StatusBadRequest)
		return false
	}
	...
	var resp WxPublicAccountNotifyResp

	if t.WxpayVerifySign(reqMap, mr.Sign) {
		Log.Write("[微信公众号支付]支付回调成功", LogInfo)
		return_type = true
		resp.Return_code = "SUCCESS"
		resp.Return_msg = "OK"
	} else {
		return_type = false
		Log.Write("[微信公众号支付]支付回失败："+"failed to verify sign, please retry!", LogErr)
	...
	if _err != nil {
		Log.Write("[微信公众号支付]回复xml编码失败,错误原因:", LogWarning)
		http.Error(w.(http.ResponseWriter), http.StatusText(http.StatusBadRequest), http.StatusBadRequest)
		return false
	}
...
func (this *pay_wxpay_publicAccount) WxpayVerifySign(needVerifyM map[string]interface{}, sign string) bool {
...
func (pay_wxpay_publicAccount) WxpayCalcSign(mReq map[string]interface{}, key string) (sign string) {
